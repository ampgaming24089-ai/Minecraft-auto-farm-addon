  ...
  for (const player of world.getAllPlayers()) {
    ...
    if (inEnd && wearsCrown(player)) {
      try {
        ...
        console.warn(`[Eternal End] could not apply crown powers: ${error}`);
      }
    }
    ...
    if (eligible) {
      try {
        ...
        console.warn(`[Eternal End] could not apply set bonus: ${error}`);
      }
      if (!wasActive) {
        try {
          player.onScreenDisplay.setActionBar("§dEternal End set §7- slow falling, resistance");
