  ...
  for (const player of world.getAllPlayers()) {
    ...
    if (inEnd && wearsCrown(player)) {
      try {
        ...
        console.warn(`[End Unbound] could not apply crown powers: ${error}`);
      }
    }
    ...
    if (eligible) {
      try {
        ...
        console.warn(`[End Unbound] could not apply set bonus: ${error}`);
      }
      if (!wasActive) {
        try {
          player.onScreenDisplay.setActionBar("§dEnd Unbound set §7- slow falling, resistance");
