  ...
  for (const player of world.getAllPlayers()) {
    ...
    if (inEnd && wearsCrown(player)) {
      try {
        ...
        console.warn(`[Endrealm] could not apply crown powers: ${error}`);
      }
    }
    ...
    if (eligible) {
      try {
        ...
        console.warn(`[Endrealm] could not apply set bonus: ${error}`);
      }
      if (!wasActive) {
        try {
          player.onScreenDisplay.setActionBar("§dEndrealm set §7- slow falling, resistance");
